...
We have three different training datasets in `dataset.py` to choose from, each serving as an example for you to use in your own code:
...
The testing dataset is the MNIST `torchvision` dataset. In the example, we explain how you can use the training data as well.
...
- dataset_type: A string from ['torchvision', 'folder', 'custom']. See `dataset.py` for more details.
